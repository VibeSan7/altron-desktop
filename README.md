...
Подготовка **0.4.0-beta.1**: от цели до результата, с отменой, восстановлением и доработкой в той же задаче. Это предварительная версия, не обещание безошибочной работы ИИ. Пока новый выпуск не опубликован, публичный раздел Releases содержит предыдущую **0.3.0-beta.1**. Точные границы проверок — в [отчёте](docs/DESKTOP_VERIFICATION.md).
...
2. Возьмите **`altron-0.4.0-beta.1.tar.gz`** из проверенной сборки; после публикации — из [Releases](https://github.com/VibeSan7/altron-desktop/releases). Не считайте архив 0.3 новой версией. Распаковывать его не нужно.
...
Встроенное обслуживание доступно начиная с 0.3. Для существующей 0.2 используйте отдельный архив `altron-maintenance-0.4.0-beta.1.tar.gz` и [пошаговую инструкцию перехода и возврата](https://github.com/VibeSan7/altron-desktop/blob/main/docs/UPDATING.md). Не импортируйте основной архив поверх прежнего профиля. Чистая установка Hermes на новом компьютере и первый вход нового аккаунта остаются за границами проверенного сценария.
