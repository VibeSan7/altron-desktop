...
- The live-model test remains skipped in ordinary suites because ordinary testing does not authorize inference. A separately authorized real-model run is recorded below.
...
## Real Desktop: authorized real-model run

On 2026-09-25, the exact public `altron-0.5.0-beta.2.tar.gz` archive passed the opt-in autonomous live test on Windows with `gpt-5.6-luna` through `openai-codex`. The tested archive SHA-256 was `077affe38d7989ca3cf9a0f54a74328f37a6cbe4cb1f7a024ac84687b48442b1`, matching the published release asset.

The run completed one interview answer and one work turn, retained native safety, and required one native approval. It produced `result.json` and `report.md` from synthetic expense values `[17, 29, 43]`; the independent verifier confirmed a count of `3`, a total of `89` RUB, unchanged input, exact file hashes, restart persistence, and no changes to the normal Desktop UI or default Hermes configuration. No credential files were copied, and no separate reviewer model was launched.

The sanitized machine-readable summary is [live-model-0.5.0-beta.2.json](evidence/live-model-0.5.0-beta.2.json). Raw application data is retained locally because it contains machine-specific paths and runtime identifiers. This run confirms that the released autonomous path can complete with one real model on the tested environment; it does not establish general model quality or replace first-time user testing.
...
These scenarios remain outside the evidence and are not replaced by fixture or single-model tests:

1. A clean-machine check of Hermes and this kit is partial: a fresh Windows Sandbox verified WebView2, Hermes bootstrap, official Node.js 22.23.2 installation, source Desktop build/launch, and profile import without replacing `default`. The bootstrap itself did not install Node or ship a packaged Desktop binary. A new account's provider login remains untested.
2. A first-time user's walkthrough using [FIRST_RUN.md](FIRST_RUN.md), including reporting confusing steps and confirming the actual result.
