...
Это инструкция к подготовленной 0.4.0-beta.1. До публикации её пакет передаётся как проверенная локальная сборка; старый публичный архив 0.3 не содержит описанных ниже новых кнопок.
