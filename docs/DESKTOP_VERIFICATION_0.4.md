...
Historical report retained for the public 0.4 release. Correction: the `0.21.3` environment label below identifies **Hermes Agent**, not the Desktop shell; the source checkout's Desktop version is `0.17.3`. See the [current report](DESKTOP_VERIFICATION.md) for separately verified environment details and later candidates.
