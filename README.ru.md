...
Публичная beta **0.5.0-beta.2** добавляет этот путь **интервью → работа → результат**. Фактические проверки и ограничения смотрите в [отчёте](docs/DESKTOP_VERIFICATION.md). Текущий интерфейс — на русском. Отдельный русский перевод есть только у README; остальная документация, формы обращений и описания выпусков — на английском.
