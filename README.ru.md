...
Локальный кандидат **0.5.0-beta.2** добавляет **интервью → работу → результат**: опишите идею, ответьте на вопросы, один раз подтвердите итоговое задание. Дальше внутренний исполнитель выполняет, проверяет и исправляет работу в согласованных пределах. Кандидат передаётся отдельными архивами и пока не заменяет [опубликованную 0.4](https://github.com/VibeSan7/altron-desktop/releases/tag/v0.4.0-beta.1). Фактические проверки и ограничения — в [отчёте](docs/DESKTOP_VERIFICATION.md).
...
2. Для этого кандидата используйте переданный **`altron-0.5.0-beta.2.tar.gz`** и соответствующий `SHA256SUMS.txt` либо соберите архив из этой ветки. Публичный архив 0.4 не содержит нового режима. Распаковывать архив не нужно; GitHub Source code ZIP не подходит.
